...
export default function AdminUserDetailPage() {
  ...
  if (loading) return <p className="text-sm text-dark-500">Loading user detail...</p>
  if (!user) return <p className="text-sm text-dark-500">User not found.</p>

  return (
    <div className="animate-fade-in">
      <div className="mb-6 flex items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold font-display text-dark-900">User Detail</h1>
          <p className="text-sm text-dark-600">api/Admin/users/{id}</p>
        </div>
        ...
        <div className="flex items-center gap-4 rounded-2xl border border-cream-200 bg-cream-50 p-4">
          ...
          <div className="min-w-0">
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              <Badge variant={user.role === 'Admin' ? 'dark' : 'default'}>{user.role || 'User'}</Badge>
              {user.levelName && <Badge variant="brand">{user.levelName}</Badge>}
              <Badge variant={user.isDeleted ? 'danger' : 'success'}>{user.isDeleted ? 'Deleted' : 'Active'}</Badge>
            </div>
          </div>
        </div>
