  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (active) setError(getApiErrorMessage(err, 'Could not load leaderboard.'))
      } finally {
  ...
  return (
    <div className="mx-auto max-w-screen-lg px-5 py-8 sm:px-6">
      <PageHeader
        eyebrow="Community"
        title="Leaderboard"
        description="Track the most consistent learners across the weekly board and all-time standings."
        action={
      ...
      {!loading && error && <EmptyState icon={Trophy} title="Leaderboard unavailable" description={error} />}

      {!loading && !error && data.length === 0 && (
        <EmptyState icon={Users} title="No rankings yet" description="Complete quizzes to start building the board." />
      )}
        ...
        <>
          <div className="mb-5 grid gap-4 md:grid-cols-3">
            {topThree.map((entry) => {
              ...
              const name = entry.fullName || entry.name || 'Learner'
              ...
              return (
                ...
                >
                  ...
                  {isMe && <p className="mt-1 text-xs font-bold uppercase tracking-normal text-brand-600">You</p>}
                  <p className="mt-4 text-2xl font-black tracking-normal text-dark-900">{Number(score).toLocaleString()} pts</p>
                </article>
              )
            })}
          </div>
                ...
                >
                  ...
                  <div className="flex min-w-0 items-center gap-3">
                    ...
                    <div className="min-w-0">
                      ...
                      {isMe && <p className="text-xs font-semibold text-brand-600">You</p>}
                    </div>
                  </div>
