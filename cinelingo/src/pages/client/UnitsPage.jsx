  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (active) setError(getApiErrorMessage(err, 'Could not load units.'))
      } finally {
  ...
  return (
    <div className="mx-auto max-w-screen-xl px-4 py-6 sm:px-6 lg:py-8">
      <PageHeader
        ...
        description="Browse film-based lessons, vocabulary sets, and quizzes aligned to your current level."
        action={
          <div className="w-full md:w-72">
            <Input placeholder="Search units..." prefix={<Search size={14} />} value={search} onChange={(e) => setSearch(e.target.value)} />
          </div>
        }
      />
          ...
          return (
            ...
            >
              ...
              <div className="p-5">
                ...
                <p className="mt-2 line-clamp-2 text-sm leading-6 text-slate-500 dark:text-slate-400">{unit.description || 'Film-based learning unit with vocabulary and quiz practice.'}</p>
                <div className="mt-4 flex items-center justify-between gap-3">
                  <span className="text-xs font-semibold uppercase tracking-normal text-slate-400 dark:text-slate-500">{unit.wordCount ?? 0} words</span>
      ...
      {!loading && filtered.length === 0 && (
        <EmptyState icon={Search} title="No units found" description="Try another search term or clear the search input." />
      )}
